...
def sample(step, num):
    ...
    #dirs = time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())
    path = os.path.join(f'GeoDiff_QM_evaluation_{dirs}', f'step{step}', 'conformation')
    os.makedirs(path, exist_ok = True)
    
    #将当前的模型也保存复制一份
    shutil.copy('Equi_Consis_Model.zip', f'GeoDiff_QM_evaluation_{dirs}')  #shutil.copy('源文件路径', '目标文件路径')
    ...
    with open('../GEMO/QM9/datasets/GeoDiff_QM9/test/ecconf_all.pickle','rb') as f:
    ...
    for key in tqdm(mgdict.keys()):
        ...
        for mg in mgdict[key]:
            target_mgdict[key].append(mg.Trans_to_Rdkitmol()) #保存真实构象
    ...
    #np.savetext
    coverage_recall_vals = [stat[2] for stat in coverage_recall]
    coverage_precision_vals = [stat[2] for stat in coverage_precision]
    ...
    print('count_error num:', count_error)
...
if __name__ == '__main__':
    ...
    s_dict = {}
    #for s in [1, 2, 5, 10, 15, 25, 30, 35, 40, 45, 50][:-4]: #new_qm加载的是25模型， new_qm2加载的15步模型，但是在测试的时候依旧采样步长为25时候的情况
    for s in [25]:
        use_ff = False
        dirs = time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())
