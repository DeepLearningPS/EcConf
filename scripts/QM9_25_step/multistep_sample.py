...
def sample(step, num):
    ...
    for stp in np.arange(step) + 1 :
        path = os.path.join(f'GeoDiff_QM9_evaluation_{dirs}', f'step{stp}', 'conformation')
        os.makedirs(path, exist_ok = True)
        step_path[stp] = path
    
    #将当前的模型也保存复制一份
    shutil.copy('Equi_Consis_Model.zip', f'GeoDiff_QM9_evaluation_{dirs}')  #shutil.copy('源文件路径', '目标文件路径')
