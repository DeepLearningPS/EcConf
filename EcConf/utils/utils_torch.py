...
def dist_deriv(x1, x2, eps=1e-7, enforce_boundaries=True, raise_warnings=True):
    ...
    """
    r = x2 - x1
    rnorm = torch.norm(r, dim=-1, keepdim=True)+1e-6
...
def angle_deriv(x1, x2, x3, eps=1e-7, enforce_boundaries=True, raise_warnings=True):
    ...
    """
    r12 = x1 - x2
    r12_norm = torch.norm(r12, dim=-1, keepdim=True)#+1e-6
    ...
    r32 = x3 - x2
    r32_norm = torch.norm(r32, dim=-1, keepdim=True)#+1e-6
...
def xyz2ic(x,zmat,mask=None,eps=1e-7):
    ...
    #dihedral=torsion_deriv(x0,x1,x2,x3)
...
def ic2xyz(dist,angle,dihedral,zmat,cond_xyz=None,mask=None,eps=1e-7):
    ...
    for i in torch.arange(1,max_atom_num,dtype=torch.int32).cuda():
        ...
        zindex=torch.index_select(zmat,dim=1,index=i).view(-1,5).long()
