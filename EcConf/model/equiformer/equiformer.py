...
from .utils import exists, default, batched_index_select, masked_mean, to_order, cast_tuple, safe_cat, fast_split
...
class Equiformer(nn.Module):
    ...
    def forward(
        ...
        rel_pos = rel_pos.masked_select(exclude_self_mask[..., None]).reshape(b, n, n - 1, coors.shape[-1])
...
class Linear_Prob(nn.Module):
    def __init__(
        ...
        self.sigma_weights = nn.ParameterList([])
        self.degrees = []

        for (degree, dim_in, dim_out) in fiber_and(fiber_in, fiber_out):
            self.mu_weights.append(nn.Parameter(torch.randn(dim_in, dim_out) / sqrt(dim_in)))
            self.sigma_weights.append(nn.Parameter(torch.randn(dim_in, dim_out) / sqrt(dim_in)))
            self.degrees.append(degree)
    def forward(self, mu,sigma):
        mu = {}
        sigma = {}
        for degree, mu_weight,sigma_weight in zip(self.degrees, self.mu_weights,self.sigma_weights):
            mu[degree] = einsum('... d m, d e -> ... e m', mu[degree], mu_weight)
            sigma[degree] = einsum('... d m, d e -> ... e m', sigma[degree], sigma_weight)
            

        return mu,sigma

#@beartype
class FeedForward_Prob(nn.Module):
    def __init__(
        self,
        fiber: Tuple[int, ...],
        fiber_out: Optional[Tuple[int, ...]] = None,
        ...
        self.prenorm     = Norm(fiber)
        self.project_in_mu  = Linear(project_in_fiber, project_in_fiber_hidden)
        self.project_in_sigma  = Linear(project_in_fiber, project_in_fiber_hidden)
        self.gate_mu        = Gate(project_in_fiber_hidden)
        self.gate_sigma        = Gate(project_in_fiber_hidden)
        self.project_out_mu = Linear(fiber_hidden, fiber_out)
        self.project_out_sigma = Linear(fiber_hidden, fiber_out)
        #print (fiber_hidden,fiber_out)
    ...
    def forward(self, features):
        ...
        mus = self.project_in_mu(outputs)
        sigmas = self.project_in_sigma(outputs)
        mus = self.gate_mu(mus)
        sigmas = self.gate_mu(sigmas)
        mus = self.project_out_mu(mus)
        sigmas = self.project_out_sigma(sigmas)
        return mus,sigmas

#@beartype
class Equiformer_Prob(nn.Module):
    def __init__(
        self,
        *,
        dim: Union[int, Tuple[int, ...]],
        ...
        **kwargs
        ...
        proj_out_klass = Linear_Prob if linear_out else FeedForward_Prob
        #print (proj_out_klass)
        self.ff_out_list=nn.ModuleList()
        for i in range(gaussian_num):
            ff_out = proj_out_klass(self.dim, (1,) * self.num_degrees) if reduce_dim_out else None
            self.ff_out_list.append(ff_out)
    ...
    def forward(
        ...
        logsigma_list=[]
        # reduce dim if specified
        if exists(self.ff_out_list):
            for i in range(self.gaussian_num):
                tmp_mu,tmp_logsigma = self.ff_out_list[i](x)
                #print (x[0][0])
                tmp_mu = {k: rearrange(v, '... 1 c -> ... c') for k, v in tmp_mu.items()}
                tmp_logsigma = {k: rearrange(v, '... 1 c -> ... c') for k, v in tmp_logsigma.items()}
                mu_list.append(tmp_mu)
                logsigma_list.append(tmp_logsigma)

        if return_pooled:
            for i in range(self.gaussian_num):
            #print ('Here are return_pooled')
                mask_fn = (lambda t: masked_mean(t, _mask, dim = 1))
                mu_list[i] = {k: mask_fn(v) for k, v in mu_list[i].items()}
                logsigma_list[i] = {k: mask_fn(v) for k, v in logsigma_list[i].items()}

        # just return type 0 and type 1 features, reduced or not
        outlist=[[],[],[]]
        for i in range(self.gaussian_num):
            pi, mu = mu_list[i][0], mu_list[i].get(1, None)
            logsigma = logsigma_list[i].get(1,None)        
            pi = rearrange(pi, '... 1 -> ...') # for type 0, just squeeze out the last dimension
            pi = pi.sum(axis=-1,keepdim=True)
            mu_ices=xyz2ic(mu,zmats)
            mu_xyz=ic2xyz(mu_ices[:,:,0],mu_ices[:,:,1],mu_ices[:,:,2],zmats)
            sigma=torch.exp(logsigma)+1e-16
            sigma_ices=xyz2ic(sigma,zmats)
            sigma_xyz=ic2xyz(sigma_ices[:,:,0],sigma_ices[:,:,1],sigma_ices[:,:,2],zmats)
            #print (pi.shape)
            mu=mu_xyz.unsqueeze(-2)
            sigma=sigma_xyz.unsqueeze(-2)
            print (mu.shape)
            outlist[0].append(pi)
            outlist[1].append(mu)
            outlist[2].append(sigma)
            #print (pi.shape,mu.shape,logsigma.shape)
        
        pi=torch.concatenate(outlist[0],axis=-1)
        mu=torch.concatenate(outlist[1],axis=-2)
        sigma=torch.concatenate(outlist[2],axis=-2)
        
        return pi,mu,sigma

    def get_mdn_possibility(self,mu,sigma,pi,pos_target,eps=1e-16):
        prob_gauss = self._get_gaussian_probability(mu, sigma, pos_target)
        prob_mdn = pi * prob_gauss
        prob_mdn = torch.sum(prob_mdn, dim=-1)
        return prob_mdn

    def _get_gaussian_probability(self,mu,sigma,pos_target,eps=1e-16):
        GAUSSIAN_COEF = 1.0 / math.sqrt(2 * math.pi)
        target = pos_target.unsqueeze(-2).expand_as(mu)
        #print (target.shape)
        errors = target - mu
        sigma = sigma + eps
        p = GAUSSIAN_COEF * torch.exp(- 0.5 * (errors / sigma)**2) / sigma
        p = p.permute(0,2,1,3).reshape((p.shape[0],p.shape[2],-1))
        #print (p.shape,p)
    ...
    def sample_batch(self,mu,sigma,pi,num):
        ...
        mu=mu.permute(0,2,1,3).reshape((mu.shape[0],mu.shape[2],-1))
        sigma=sigma.permute(0,2,1,3).reshape((sigma.shape[0],sigma.shape[2],-1))
        mu_sample=mu[index_batch,index_cats]
        sigma_sample=sigma[index_batch,index_cats]+0.5
        values=torch.normal(mu_sample,sigma_sample)
        values=values.view(-1,n,d)
        return values
...
# @beartype
class Equiformer_Consistency(nn.Module):
    def __init__(
        self,
        *,
        dim: Union[int, Tuple[int, ...]],
        ...
        **kwargs
        ...
        self.feature_embed = nn.Sequential(
            nn.Linear(token_dim,type0_feat_dim*4),
            nn.SiLU(),
            nn.Linear(type0_feat_dim*4,type0_feat_dim)
        ) if exists(token_dim) else None 
        
        self.sigma_embed = nn.Sequential(
            nn.Linear(1, type0_feat_dim*4),
            nn.SiLU(),
            nn.Linear(type0_feat_dim*4, type0_feat_dim),
        )
        # positional embedding
    ...
    def forward(
        ...
        if exists(self.token_emb):
            ...
            feats = self.feature_embed(feats)

        if exists(self.pos_emb):
            assert feats.shape[1] <= self.num_positions, 'feature sequence length must be less than the number of positions given at init'
            feats = feats + self.pos_emb(torch.arange(feats.shape[1], device = feats.device))

        if exists(self.sigma_embed):
            sigma_emb=self.sigma_embed(sigmas.view(-1,1)).unsqueeze(1)
            #print (sigma_emb.shape,feats.shape)
            feats = feats + sigma_emb
